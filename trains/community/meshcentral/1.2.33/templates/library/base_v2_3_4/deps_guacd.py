...
class GuacdConfig(TypedDict):
    port: NotRequired[int]


SUPPORTED_REPOS = ["guacamole/guacd"]


class GuacdContainer:
    def __init__(self, render_instance: "Render", name: str, image: str, config: GuacdConfig):
        ...
        c.set_command(["-l", str(self.get_port())])
    ...
    def _get_repo(self, image):
        ...
        if repo not in SUPPORTED_REPOS:
            raise RenderError(f"Unsupported repo [{repo}] for guacd. Supported repos: {', '.join(SUPPORTED_REPOS)}")
        return repo

    def get_port(self):
        return self._config.get("port") or 4822
