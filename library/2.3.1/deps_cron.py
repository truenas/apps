...
class CronConfig(TypedDict):
    tasks: list[TasksConfig]


SUPPORTED_REPOS = ["ixsystems/cron"]


class CronContainer:

    def __init__(self, render_instance: "Render", name: str, image: str, config: CronConfig):
        ...
        c.environment.add_env("METRICS_PORT", 8080)
        c.healthcheck.set_test("curl", {"port": 8080, "path": "/health"})
        c.remove_devices()
        c.set_grace_period(60)

        if not self._config.get("tasks"):
            raise RenderError("Cron tasks not defined")
