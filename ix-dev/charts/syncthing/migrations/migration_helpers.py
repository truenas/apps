...
def migrate_dns_config(dns_config):
    ...
    for opt in dns_config.get("options", []):
        dns_opts.append(f"{opt.key}:{opt.value}")
...
def migrate_host_path_type(host_path):
    ...
    result = {
        "type": "host_path",
        "host_path_config": {
            "acl_enable": host_path.get("aclEnable", False),
        },
    }

    if host_path.get("aclEnable", False):
        result["host_path_config"].update(
            {"acl": migrate_acl_entries(host_path.get("acl", {}))}
        )
    else:
        result["host_path_config"].update({"host_path": host_path["hostPath"]})
...
def migrate_storage_item(storage_item):
    ...
    if storage_item["type"] == "ixVolume":
        ...
        pass
        # return migrate_host_path_type(storage_item)
